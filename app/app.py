...
class Application(tornado.web.Application):
    def __init__(self):
        ...
        handlers.extend([
            (r"/badges", controllers.badges.GetBadgesHandler,
                dict(badges=badges, badgeslist=badgeslist,
                     badgestree=badgestree)),
            (r"/badges/tree", controllers.badges.GetBadgesTreeHandler,
                dict(badges=badges, badgeslist=badgeslist,
                     badgestree=badgestree)),
            (r"/profile/([0-9]+)/badges",
                controllers.badges.GetBadgesForUserHandler,
                dict(badgesusers=badgesusers)),
            (r"/profile/([0-9]+)/badges/([0-9]+)",
                controllers.badges.AddChangeDeleteBadgesForUserHandler,
                dict(badgesusers=badgesusers))
        ])
        ...
        handlers.extend([
            (r"/profile/([0-9]+)/baskets",
                controllers.baskets.GetChangeOrderBasketsHandler,
                dict(dbp=baskets_dbp)),
            (r"/profile/([0-9]+)/basket",
                controllers.baskets.AddBasketHandler,
                dict(dbp=baskets_dbp)),
            (r"/profile/([0-9]+)/basket/([0-9]+)",
                controllers.baskets.GetDelChangeOrderBasketHandler,
                dict(dbp=baskets_dbp)),
            (r"/profile/([0-9]+)/basket/([0-9]+)/([0-9]+)",
                controllers.baskets.AddDelUserFromBasketHandler,
                dict(dbp=baskets_dbp)),
        ])
        ...
        handlers.extend([
            (r"/profile/([0-9]+)/blacklist",
                controllers.blacklist.GetBlacklistHandler,
                dict(blacklist2db=blacklist2db, blacklist2dic=blacklist2dic)),
            (r"/profile/([0-9]+)/blacklist/([0-9]+)",
                controllers.blacklist.AddDeleteBlacklistedForUserHandler,
                dict(blacklist2db=blacklist2db, blacklist2dic=blacklist2dic)),
        ])
        ...
        handlers.extend([
            (r"/profile/([0-9]+)/friends",
                controllers.friends.GetFriendsHandler,
                dict(friends=friends)),
            (r"/profile/([0-9]+)/friends/([0-9]+)",
                controllers.friends.AddDeleteFriendHandler,
                dict(friends=friends)),
            (r"/profile/([0-9]+)/friends/approve/([0-9]+)",
                controllers.friends.ApproveFriendHandler,
                dict(friends=friends)),
        ])
        ...
        handlers.extend([
            (r"/profile/([0-9]+)/photos",
                controllers.images.GetAllProfileImagesHandler,
                dict(profileimages=profileimages, images_path=images_path)),
            (r"/profile/([0-9]+)/photos/friend",
                controllers.images.GetProfileImagesByTypeHandler,
                dict(profileimages=profileimages, images_path=images_path,
                     t='friend')),
            (r"/profile/([0-9]+)/photos/profile",
                controllers.images.GetProfileImagesByTypeHandler,
                dict(profileimages=profileimages, images_path=images_path,
                     t='profile')),
            (r"/profile/([0-9]+)/photos/other",
                controllers.images.GetProfileImagesByTypeHandler,
                dict(profileimages=profileimages, images_path=images_path,
                     t='other')),
            (r"" + images_path + "/(.*)",
                tornado.web.StaticFileHandler,
                {"path": conf['env']['static_path'] +
                    conf['images']['images_root']}),
        ])
        ...
        handlers.extend([
            (r"/profile/([0-9]+)/recommended/([0-9]+)/([0-9]+)",
             controllers.recommended.GetRecommendedHandler, dict(
                 dbp=recommended_dbp)),
        ])
        ...
        handlers.extend([
                        (r"/event/",
                            controllers.events.EventsHandler,
                            dict(db=db)),
                       (r"/event/([0-9]+)",
                           controllers.events.EventHandler,
                           dict(db=db)),
                       (r"/event/([0-9]+)/user/([0-9]+)",
                           controllers.events.EventUserHandler,
                           dict(db=db)),
                        ])
        ...
        handlers.extend([
            (r"/profile/([0-9]+)/status/all",
                controllers.status.GetStatusesHandler,
                dict(dbp=status_dbp)),
            (r"/profile/([0-9]+)/status/last",
                controllers.status.GetLastStatusHandler,
                dict(dbp=status_dbp)),
            (r"/profile/([0-9]+)/status/add",
                controllers.status.AddStatusHandler,
                dict(dbp=status_dbp)),
            (r"/profile/([0-9]+)/status/([0-9]+)/delete",
                controllers.status.DeleteStatusHandler,
                dict(dbp=status_dbp)),
        ])
