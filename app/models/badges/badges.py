...
class BadgesUsers(ExceptionLogger):
    ...
    def get_users_by_badges(self, badges_ids):
        """Returns all user_ids that have a connection
        with one of the badge_id's from list badges_ids"""
        try:
            result = self.table.select(
                self.table.c.badge_id.in_(badges_ids)).execute()
    ...
    def addchange_user_badge(self, user_id, badge_id, desc=None, visibility=0):
        ...
        try:
            self.table.insert().values(badge_id=badge_id,
                                       user_id=user_id,
                                       description=desc,
                                       visibility=visibility).execute()
...
class Badges(ExceptionLogger):
    ...
    def __genTreeRec__(self, x, parent_ids, R):
        ...
        if x['badge_id'] not in parent_ids:
            ...
            return({
                'node': x,
                'children': [self.__genTreeRec__(x, parent_ids, R)
                             for x in R[x['badge_id']]]
            })
